...
**The goal is deliberately not part of this module.** It rides on no
counter at all: ``client/client.py``'s ``_handle_check_goal`` reads the
current area out of game memory (``constants.GAME_END_AREA_INDICES``), so
no counter amount can ever be mistaken for a goal signal. This module only
