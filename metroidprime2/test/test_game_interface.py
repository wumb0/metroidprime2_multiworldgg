...
class TestConsumeCounters(unittest.TestCase):
    ...
    must route those through ``_wide_decrement_patch`` instead, while still
    using the OPR-provided (narrow) patch for the goal counter. Exercised
    ...
    *assembly* time (``Instruction.compose``'s range asserts), not at some
    ...
    def test_consume_counters_handles_goal_and_pickup_counters_in_one_body(self) -> None:
        from .. import constants

        interface, fake = self._prepared_interface()
        pickup_item = constants.PICKUP_COUNTER_ITEMS[0]
        full_value = (1 << constants.BITS_PER_COUNTER) - 1

        leftovers = interface.consume_counters(
            [
                (constants.GOAL_COUNTER_ITEM, -constants.GOAL_SIGNAL_AMOUNT),
                (pickup_item, -full_value),
            ]
        )
