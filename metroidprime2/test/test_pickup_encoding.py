...
def _empty_inventory() -> dict[int, tuple[int, int]]:
    return dict.fromkeys(constants.ALL_COUNTER_ITEMS, (0, 0))
...
def _reserved_item_ids() -> set[int]:
    ...
    collide with, short of the one deliberate exception
    (``constants.GOAL_COUNTER_ITEM`` -- see the next test).
...
class TestReservedItemIdsDisjoint(unittest.TestCase):
    ...
    def test_goal_counter_item_is_reserved_specifically_as_multiworld(self) -> None:
        # GOAL_COUNTER_ITEM (74) IS in the reserved set -- that's the
        # point, not an oversight: it's randovania's own "Multiworld"
        # allocation, which is exactly why this world may use it. Assert
        # that specific fact rather than exempting the whole check, so a
        # future accidental reassignment to some OTHER reserved id would
        # still fail loudly.
        header = load_json("logic_database/header.json")
        multiworld_entry = header["resource_database"]["items"]["Multiworld"]
        self.assertEqual(constants.GOAL_COUNTER_ITEM, multiworld_entry["extra"]["item_id"])
        self.assertIn(constants.GOAL_COUNTER_ITEM, _reserved_item_ids())
