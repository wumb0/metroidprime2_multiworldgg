...
class TestManualGrantAppendedLikeRealItem(unittest.TestCase):
    """Regression test for the historical ``/grant_item`` bug (PLAN.md
    section N; the command itself was later removed entirely in favor of
    the server's own host.yaml-gated ``!getitem`` cheat, section O): a
    manually granted item used to be looked up in ITEM_TABLE and have its
    raw ``gains`` pushed straight to game memory, bypassing this module
    entirely. For a cross-computed item that permanently stranded the
    player: a Missile Launcher granted this way after 8 Missile Expansions
    had already been received applied raw gains of 5 missile capacity, but
