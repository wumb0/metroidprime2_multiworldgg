...
A port of the Metroid Prime 1 randomizer's ``warpToStart``
(``MultiWorldGG/worlds/metroidprime/Config.py``, hardcoded on there), which
``open-prime-rando`` has no equivalent for. Like randomprime's, it is two
halves:
...
**DOL** (`apply_dol_patches`): the save-station Think broadcasts
...
The hook address differs per build and is found by pattern matching --
see ``metroidprime2/tools/find_warp_addresses.py``.
"""
...
if TYPE_CHECKING:
    ...
    from .versions import WarpToStartAddresses


_LAYER_NAME = "AP Warp To Start"
...
def find_save_station_special_function(area: Area) -> Any | None:
    ...
    if len(matches) > 1:
        raise ValueError(
            f"{area.name} has {len(matches)} save-station SpecialFunctions; expected at most one"
        )
    return matches[0] if matches else None


def add_warp_to_start(editor: Any, mlvl: Any, area: Area, starting_area: AreaReference) -> None:
    ...
    from retro_data_structures.properties.echoes.objects import HUDMemo, Timer, WorldTeleporter
    ...
    memo = layer.add_instance_with(
        HUDMemo(
            editor_properties=EditorProperties(name="AP Warp Memo"),
    ...
    timer = layer.add_instance_with(
        Timer(
            editor_properties=EditorProperties(name="AP Warp Delay"),
    ...
    teleporter = layer.add_instance_with(
        WorldTeleporter(
            editor_properties=EditorProperties(name="AP Warp To Start"),
...
def patch_save_prompt_hint(editor: Any) -> None:
...
def register(area_patcher: AreaPatcher, starting_area: AreaReference) -> None:
    ...
    of the 18 Save Station rooms: open-prime-rando already runs three global
    functions, so every area is loaded and walked regardless, and a
    discovered list can't drift out of sync with the game.
    """
    area_patcher.add_global_function(
        functools.partial(add_warp_to_start, starting_area=starting_area)
    )
...
def build_gate_cave(addresses: WarpToStartAddresses) -> list[BaseInstruction]:
    ...
    On entry the save-station Think has already set up the broadcast call:
    ...
    broadcast helper so it returns straight to the Think -- no stack frame,
    and r0/r11/r12 are volatile and not arguments.
    """
    from ppc_asm.assembler.ppc import addi, b, ble, cmpw, lis, lwz, ori, r0, r4, r5, r11, r12
...
def apply_dol_patches(cave: CodeCaveTracker, addresses: WarpToStartAddresses) -> None:
    ...
    open-prime-rando's ``_apply_patches``.
    """
